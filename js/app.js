// Grab the width of the SVG box
var width = parseInt(d3.select("#scatter").style("width"));

// Designate the height of the graph
var height = width - width / 3.8;

// Margin spacing for graph anl label
var margin = 30;
var labelArea = 100;
...
// Use xText to append three text SVGs, with y coordinates specified to space out the values.
// Poverty
xText
  .append("text")
  .attr("y", -30)
...
function visualize(theData) {
  ...
  // Designate the defaults here, which carry the same names
  // as the headings in their matching .csv data file.
  var curX = "poverty";
  var curY = "obesity";

  // Save empty variables for the min and max values of x and y.
  // Alter the values in functions and remove repititious code.
  var xMin;
  var xMax;
  var yMin;
  var yMax;
  ...
  // Append the circles for each row of each state
  theCircles
    .append("circle")
  ...
  d3.selectAll(".aText").on("click", function() {
    // Make sure Save a selection of the clicked text, to reference it without typing out the invoker each time.
    var self = d3.select(this);

    // Run this on inactive labels only.
    // It's a waste of the processor to execute the function, if the data is already displayed on the graph.
